...
const get = async (params = {}, offset = 0, limit = 20) => {
  try {
    const res = await db.collection('ecb_account')
    .where(params)
    .skip(offset)
    .limit(limit)
    .get()
...
const getGroupByCategory = async (params = {}) => {
  try {
    const res = await db.collection('ecb_account').aggregate()
      .group({
        _id: '$category',
        total: { $sum: 1 },
      })
...
const count = async (params = {}) => {
  try {
    const res = await db.collection('ecb_account').where(params).count()
...
const add = async (data) => {
  try {
    const res = await db.collection('ecb_account').add({ data })
...
const remove = async id => {
  try {
    const res = await db.collection('ecb_account').doc(id).remove()
